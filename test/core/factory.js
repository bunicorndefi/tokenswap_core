...
contract('BFactory', async (accounts) => {
    ...
    describe('Factory', () => {
        ...
        it(' on non pool returns false', async () => {
        ...
        it('nonadmin cant set klabs address', async () => {
            await truffleAssert.reverts(factory.setBLabs(nonAdmin, { from: nonAdmin }), 'ERR_NOT_blabs');
        });

        it('admin changes klabs address', async () => {
            await factory.setBLabs(user2);
            const klab = await factory.getKLabs();
            assert.equal(klab, user2);
        });
    });
});
