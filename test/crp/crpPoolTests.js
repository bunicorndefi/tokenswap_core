...
contract('crpPoolTests', async (accounts) => {
    ...
    const NAME = "Buni Smart Pool Custom Name";
    ...
    it('crpPool should have no BPool before creation', async () => {
        ...
        assert.equal(kPoolAddr, ZERO_ADDRESS);
    });
    ...
    it('BPool should have initial token balances', async () => {
        ...
        const kPoolXYZBalance = await xyz.balanceOf.call(kPoolAddr);
        const adminWethBalance = await weth.balanceOf.call(admin);
        const kPoolWethBalance = await weth.balanceOf.call(kPoolAddr);
        const adminDaiBalance = await dai.balanceOf.call(admin);
        const kPoolDaiBalance = await dai.balanceOf.call(kPoolAddr);
    ...
    it('JoinPool should not revert if smart pool is finalized', async () => {
        ...
        let previouskPoolXyzBalance = await xyz.balanceOf.call(kPoolAddr);
        let previouskPoolWethBalance = await weth.balanceOf.call(kPoolAddr);
        let previouskPoolDaiBalance = await dai.balanceOf.call(kPoolAddr);
        previouskPoolXyzBalance = Decimal(fromWei(previouskPoolXyzBalance));
        previouskPoolWethBalance = Decimal(fromWei(previouskPoolWethBalance));
        previouskPoolDaiBalance = Decimal(fromWei(previouskPoolDaiBalance));
        ...
        let balanceChange = (Decimal(poolAmountOut).div(previousPoolBalance)).mul(previouskPoolWethBalance);
        const currentWethBalance = previouskPoolWethBalance.add(balanceChange);
        balanceChange = (Decimal(poolAmountOut).div(previousPoolBalance)).mul(previouskPoolDaiBalance);
        const currentDaiBalance = previouskPoolDaiBalance.add(balanceChange);
        balanceChange = (Decimal(poolAmountOut).div(previousPoolBalance)).mul(previouskPoolXyzBalance);
        const currentXyzBalance = previouskPoolXyzBalance.add(balanceChange);

        assert.equal(fromWei(balance), currentPoolBalance);
        assert.equal(kPoolXYZBalance, toWei(String(currentXyzBalance)));
        assert.equal(kPoolWethBalance, toWei(String(currentWethBalance)));
        assert.equal(kPoolDaiBalance, toWei(String(currentDaiBalance)));
    });
    ...
    it('Should get the BuniSafeMath address', async () => {
        const mathLibrary = await crpPool.getBuniSafeMathVersion();
    ...
    it('should exitpool', async () => {
        ...
        // Balances of all tokens increase proportionally to the pool balance
        let balanceChange = (Decimal(poolAmountIn).div(previousPoolBalance)).mul(previouskPoolWethBalance);
        const currentWethBalance = previouskPoolWethBalance.sub(balanceChange);
        balanceChange = (Decimal(poolAmountIn).div(previousPoolBalance)).mul(previouskPoolDaiBalance);
        const currentDaiBalance = previouskPoolDaiBalance.sub(balanceChange);
        balanceChange = (Decimal(poolAmountIn).div(previousPoolBalance)).mul(previouskPoolXyzBalance);
        const currentXyzBalance = previouskPoolXyzBalance.sub(balanceChange);

        let relDif = calcRelativeDiff(currentXyzBalance, fromWei(kPoolXYZBalance));
        assert.isAtMost(relDif.toNumber(), errorDelta);
        relDif = calcRelativeDiff(currentDaiBalance, fromWei(kPoolDaiBalance));
        assert.isAtMost(relDif.toNumber(), errorDelta);
        relDif = calcRelativeDiff(currentWethBalance, fromWei(kPoolWethBalance));
        assert.isAtMost(relDif.toNumber(), errorDelta);
    ...
    describe('PCToken interactions', () => {
        it('Token descriptors', async () => {
            ...
            assert.equal(name, NAME); // 'Buni Smart Pool-' + SYMBOL);
